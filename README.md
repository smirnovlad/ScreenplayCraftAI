...
Перед тем как запускать программу, необходимо зарегистрироваться на платформе Mistral AI и получить API ключ. Сделать это может бесплатно любой желающий, следуя [официальной инструкции](https://docs.mistral.ai/getting-started/quickstart/#account-setup). Полученный ключ необходимо скопировать в [config.py](config.py). 
...
**Замечание.** Для генерация сценариев в скрипте [main.py](main.py) используется бейзлайн-модель (см. Attempt 1 ниже).
